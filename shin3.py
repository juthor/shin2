...
#웹페이지 열고 소스코드 읽기
response=requests.get("https://www.shinhancard.com/conts/person/card_info/dream/credit/life/1540955_46588.jsp")
...
#cardlist=[each_line.get_text().strip() for each_line in cardlist[:]]
...
for x in range (len(keywords)):
    #dotdetail=soup.find('div.cardLookBg63>ul.listDotHyphen>li')
    dotdetail=soup.findall('ul', class_="listDotHyphen")
    temp=str(keywords[x])+"-"+str(dotdetail)+","+"\n"
    print(temp)

dotdetails=soup.select('div.cardLookBg63>ul.listDotHyphen>li')
for dotdetails in detail:
    print(deta)
